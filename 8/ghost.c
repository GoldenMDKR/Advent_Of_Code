...
int main(){
    ...
    char all_a[10][300] = {[0 ... 9][0 ... 299] = '\0'};
    long all_first[6] = {[0 ... 5] = 0};
    ...
    {
        step = 0;
        struct pair* curr_pair = htab_get(h_tab,all_a[j]);
        i = 0;
        while(curr_pair->key[2] != 'Z')
        {
            if (left_right[i] == 'L')
    ...
    for (i = 0; i<j; i++)
        printf("%li, ", all_first[i]);
    printf("\n");

    long lowest = findlcm(all_first, j);
